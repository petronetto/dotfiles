---
name: define
description: Write a spec (PRD) for a feature or fix before coding, by interviewing the user one question at a time until shared understanding. Trigger on "spec", "requirements", "design the work", "scope it first". Produces the spec that `plan` decomposes; outputs Markdown only, never code.
argument-hint: "<feature or fix description>"
---

# Define

Define what to build and why before any code. Interview the user one question at a time until you reach shared understanding, surface assumptions explicitly, and write the spec that `plan` decomposes into steps. The spec lives on disk and is the shared source of truth. Output is Markdown only; implementation is separate (`build`).
...
- Never delete an existing spec. Ask before resuming a spec that already has unfinished work or starting a new one.
...
```
`<task-name>` is a short git-safe slug. Create it if missing. If it already holds an unfinished spec, ask whether to resume or start a new one.
...
### 6. Write the spec
Fill the template at `assets/summary-file.md` into `.plans/<branch>/<task-name>/summary.md`. This is the PRD/spec for the work. Cover, at minimum, problem and goals, non-goals, boundaries (Always / Ask first / Never), approach, the project commands, and the key decisions with why. Step files link back to it instead of restating it.

### 7. Present and stop
Summarize the spec and list the file created. Do not implement. Hand off to `plan` only after approval.

## Rationalizations

| Excuse | Reality |
| --- | --- |
| "This is simple, no spec needed." | Simple work still needs a few lines of acceptance criteria and boundaries. A two-line spec is fine. |
| "I'll write the spec after the code." | That's documentation, not specification. The spec's value is forcing clarity before code exists. |
| "The user knows what they want." | Even clear requests carry implicit assumptions. Surfacing them up front is the whole point. |
| "It's one big feature; I'll keep it as one spec." | If acceptance criteria cluster into independently testable groups, a monolithic spec forces every step to reason over the whole contract. A small capability map is the cheap alternative. |
| "Planning is overhead." | Planning is the task. A 15-minute spec prevents hours of rework. |
...
- One spec whose scope spans several independently testable capabilities with no capability map.
...
- [ ] `summary.md` is saved under `.plans/<branch>/<task-name>/`.

## References

- Design and code-quality standards: `references/code-quality.md`
- Spec template: `assets/summary-file.md`
